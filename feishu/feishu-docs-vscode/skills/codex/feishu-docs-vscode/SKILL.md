...
description: Use this skill whenever the user mentions Feishu/Lark cloud docs, 飞书云文档, docx/wiki links, Drive Markdown files, syncing a Feishu document into VSCode, previewing rendered Markdown articles, or asks Codex to read, sync, preview, edit, push, or build a VSCode workflow around Feishu docs. Prefer doing the workflow directly and only ask the user for Feishu authorization, a missing document link, or missing permissions.
metadata:
  requires:
    bins: ["lark-cli", "node", "npm"]
---

# Feishu Docs VSCode

Use this skill to connect Feishu cloud documents to a Codex/VSCode workspace as local Markdown plus a rendered article preview.
...
- In Codex sandboxed runs, Feishu commands may need elevated execution because macOS Keychain and network access can be restricted.
...
## Sandbox and Keychain

If a Feishu command fails with:

```text
keychain Get failed: keychain not initialized
```

rerun the command with Codex elevated execution. Only suggest `lark-cli config keychain-downgrade` if the user explicitly wants a non-Keychain setup and understands that local file permissions replace macOS Keychain protections.
